...
In this case, we retain the primary key relation, 
 but drop the entries in all other relations.
...
%
\begin{lstlisting}
SELECT Address FROM R WHERE R.Author = "Codd"
\end{lstlisting}
%
...
The join with \lstinline|R_id| can be omitted, 
...
The reason we need a {\em set} of queries is that
 the output of the original query under 3-valued logic semantics may 
 also contain nulls, 
...
Finally, \lstinline|expand| renames attributes as above, 
 ...
 introduce a new predicate $R_a.\text{id} = R_\text{id}.\text{id}$,
...
Of course, the Columnar Semantics does not 
 ...
 in the presence of nulls.
We will show exactly when the two semantics disagree
 in Section~\ref{sec:cv3}.
\end{example}
...
When we evaluate a query, the \lstinline|WHERE| clause 
 only evaluates to true when the all values of each attribute
...
During expansion, we desugar \lstinline|R MISSING a|
 to \lstinline|R$_\texttt{id}$.id NOT IN R$_\texttt{a}$.id|.
...
\end{lstlisting}
Under 2-valued logic semantics, 
...
We would argue that the result produced 
 by Columnar Semantics is more intuitive;
...
Then, for each negated predicate \lstinline|NOT P(R.x, S.y)|
 we add to the expanded set a new query where the predicate is 
 replaced by \lstinline|(R MISSING x) OR (S MISSING y)|. 
This simulates the behavior of \lstinline|NOT P(R.x, S.y)|
 under 2-valued logic semantics, 
 where it returns true when either argument is null.

In our solution above, we used an expansion scheme 
 that could potentially generate an exponential number of queries
 in the number of negated predicates.
This was done to provide intuition
 for our claim that Columnar Semantics captures the 2-valued logic semantics,
 and we leave to future work to find a more efficient expansion scheme.
On the other hand, it may be acceptable
 that 2-valued logic queries with negations 
 correspond to a large number of expanded queries in Columnar Semantics.
...
One immediate task is to formally prove our claims
 in Section~\ref{sec:cv3}, 
 ideally with an expansion scheme that produces 
 queries of linear size.
We thus state the following conjecture:
%
\begin{conjecture}
For every query $Q^{3v}$ in the standard 3-valued semantics, 
 there is a set of expanded queries $\vec{Q}^{cs}$ in Columnar Semantics,
 where the size of $\vec{Q}^{cs}$ is linear in the size of $Q^{3v}$,
 and $\vec{Q}^{cs}$ simulates $Q^{3v}$.
In the converse, for every (non-expanded) $Q^{cs}$ query in Columnar Semantics, 
 there is a $Q^{3v}$ query in the standard 3-valued semantics,
 where the size of $Q^{3v}$ is linear in the size of $\vec{Q}^{cs}$,
  and $Q^{3v}$ simulates $\vec{Q}^{cs}$.
\end{conjecture}
%
The first direction ensures we can express every SQL query 
 in Columnar Semantics succintly, 
 and the other direction ensures we can compile
 every query in Columnar Semantics into the standard semantics
 efficiently.
This is remniscient of the result by Peterfreund and Libkin~\cite{DBLP:conf/pods/LibkinP23},
 that their 2-valued semantics can be efficiently mapped to the standard semantics and back.

Even with a linear expansion scheme, 
 it is far from obvious that 
 we can always compile every Columnar Semantics query 
 into an optimal 3-valued logic query.
...
As a side effect of its design, the Columnar Semantics
 may be more amenable to modern query processing techniques.
...
As such architecture already stores tables by columns, 
 Columnar Semantics may even be ``closer to the metal'' 
...
We think of the approach such existing systems
