...
{\color{red} We are actually linear. Test Columnar Semantics .}
...
%
{\color{red} Move the conjecture to Section~\ref{sec:cv3}}
%
\begin{conjecture}
...
In the converse, for every (non-expanded) $Q^{cs}$ query in Columnar Semantics, 
 ...
 where the size of $Q^{3v}$ is linear in the size of $Q^{cs}$,
  and $Q^{3v}$ simulates $Q^{cs}$.
\end{conjecture}
%
